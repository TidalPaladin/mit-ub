...
__global__ void posterize_kernel(const scalar_t *__restrict__ input, scalar_t *__restrict__ output,
                                 const float posterize_prob, const int bits, const int64_t batch_size,
    ...
    if (apply_posterize) {
        ...
        // First scale to [0, levels], round to nearest integer, then scale back to [0, 1]
...
torch::Tensor posterize(const torch::Tensor &input, const float posterize_prob, const int bits, const int64_t seed) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::ScalarType::Half, at::ScalarType::BFloat16, input.scalar_type(), "posterize", ([&] {
            int min_grid_size;
            int block_size;
            cudaOccupancyMaxPotentialBlockSize(&min_grid_size, &block_size, (void *)posterize_kernel<scalar_t>, 0, 0);
            const unsigned int blocks_x = (seq_len + block_size - 1) / block_size;
            const dim3 blocks(blocks_x, batch_size);

            posterize_kernel<scalar_t><<<blocks, block_size>>>(input.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(),
...
void posterize_(torch::Tensor &input, const float posterize_prob, const int bits, const int64_t seed) {
    // Validate bits parameter (typically between 1 and 8)
    TORCH_CHECK(bits >= 1 && bits <= 8, "Number of bits must be between 1 and 8");
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::ScalarType::Half, at::ScalarType::BFloat16, input.scalar_type(), "posterize", ([&] {
            ...
            posterize_kernel<scalar_t><<<blocks, block_size>>>(input.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(),
                                                               posterize_prob, bits, batch_size, seq_len, seed);
        }));
}
