...
__global__ void invert_kernel(const scalar_t *__restrict__ input, scalar_t *__restrict__ output,
                              const float invert_prob, const float solarize_prob, const float solarize_threshold,
    ...
    // Decide if this batch entry should be solarized (using independent RNG state)
...
torch::Tensor invert(const torch::Tensor &input, const float invert_prob, const float solarize_prob,
                     const float solarize_threshold, const int64_t seed) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::ScalarType::Half, at::ScalarType::BFloat16, input.scalar_type(), "invert", ([&] {
            ...
            invert_kernel<scalar_t><<<blocks, block_size>>>(input.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(),
                                                            invert_prob, solarize_prob, solarize_threshold, batch_size,
                                                            seq_len, seed);
        }));
...
void invert_(torch::Tensor &input, const float invert_prob, const float solarize_prob, const float solarize_threshold,
             const int64_t seed) {
    // Prepare output and infer dimensions
    const int64_t batch_size = input.size(0);
    const int64_t seq_len = input.numel() / batch_size;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("invert_", &invert_, "Invert operation with optional solarize in-place");
}
