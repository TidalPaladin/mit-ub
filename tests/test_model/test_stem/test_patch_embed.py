...
class TestPatchEmbed2d:
    ...
    def test_extra_repr(self):
        ...
        result = str(layer)
        exp = "PatchEmbed2d(\n  in=3, embed=64, patch_size=(4, 4)\n  (pos_enc): RelativeFactorizedPosition(in=2, hidden=256, out=64, dropout=0.0, act=relu2, norm=False, norm_type=layernorm)\n)"
        assert result == exp
...
class TestPatchEmbed3d:
    ...
    def test_extra_repr(self):
        ...
        result = str(layer)
        exp = "PatchEmbed3d(\n  in=3, embed=64, patch_size=(4, 4, 4)\n  (pos_enc): RelativeFactorizedPosition(in=3, hidden=256, out=64, dropout=0.0, act=relu2, norm=False, norm_type=layernorm)\n)"
        assert result == exp
