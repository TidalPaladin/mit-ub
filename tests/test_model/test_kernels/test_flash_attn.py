...
EASY_SHAPE_PARAMS: Final = (
    ...
    pytest.param(1, 32, 32, 48, 1, id="b=1,lq=32,lk=32,dhead=48,nhead=1"),
...
HARD_SHAPE_PARAMS: Final = (
    pytest.param(4, 128, 256, 48, 4, id="b=4,lq=128,lk=256,dhead=48,nhead=4"),
...
def test_flash_attn_forward_bias(b, lq, lk, dhead, nhead, dpos, dtype, atol, slope):
    ...
    pos_k = torch.randn((b, lk, nhead, dpos), device="cuda", dtype=dtype)
    ...
    baseline_output = F.scaled_dot_product_attention(
        q.movedim(1, 2), k.movedim(1, 2), v.movedim(1, 2), attn_mask=bias
    ).movedim(2, 1)
...
def test_flash_attn_backward(b, lq, lk, dhead, nhead, dtype, atol):
    ...
    # Baseline
    o = F.scaled_dot_product_attention(q.movedim(1, 2), k.movedim(1, 2), v.movedim(1, 2)).movedim(2, 1)
    o.sum().backward()
