...
def euclidean_distance(a, b):
    ...
    cast(Any, euclidean_distance_kernel)[grid](
        a,
        b,
        c,
        M,
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("-K", type=int, default=2, help="Fixed size of K dimension")
...
def main(args: Namespace):
    ...
    total_tests = 2 * len(test_configs)
    ...
    benchmark = triton.testing.perf_report(
        triton.testing.Benchmark(
            ...
            styles=[("green", "-"), ("blue", "-")],
            ylabel="TFLOPS",
            plot_name="euclidean-kernel",
            args={},
        )
    )(partial(_benchmark, K=args.K, warmup=args.warmup, rep=args.rep))
    df: Any = benchmark.run(
        show_plots=False, print_data=False, return_df=True, save_path=args.output, bar=bar, verbose=args.verbose
    )
    bar.close()
