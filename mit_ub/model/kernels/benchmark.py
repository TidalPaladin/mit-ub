...
class Dtype(StrEnum):
    ...
    def torch(self) -> torch.dtype:
        match self:
            ...
            case _:
                raise TypeError(f"Unsupported dtype {self}")
...
class DimConfig:
    ...
    def from_namespace(cls: Type[Self], args: Namespace, name: str) -> Self:
        if name in args:
            ...
            raise ValueError(f"Cannot find values for {name} dimension.")
...
class KernelExecutor:
    ...
    def randn(self, *args, **kwargs) -> Tensor:
        ...
        result = torch.randn(*args, device="cuda", **kwargs)
...
class CLI:
    ...
    def __post_init__(self):
        if self.output is not None and not self.output.is_dir():
            raise NotADirectoryError(f"Output path {self.output} is not a directory")
        if not self.kernels:
    ...
    def __call__(self, endpoint: Endpoint) -> None:
        ...
        match self.line_arg:
            ...
            case _:
                raise ValueError(f"Unsupported line arg {self.line_arg}")
        ...
        def func(
            ...
            **kwargs,
            ...
            if endpoint == Endpoint.PROFILE:
                kernel.profile(mode, dtype, **kwargs)
            elif endpoint == Endpoint.BENCHMARK:
                return kernel.benchmark(mode, dtype, **kwargs)
            else:
